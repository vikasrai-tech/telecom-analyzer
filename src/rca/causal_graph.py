"""
Causal Graph — 5G Protocol Stack

Defines the directed acyclic graph (DAG) of protocol layers and the
causal rules used by the RCA engine.

Layer ordering (lowest = root, highest = symptom):
  0  PHY/L1       — physical radio (BLER, SNR, RSRP)
  1  MAC/RLC/PDCP — L2 scheduling (HARQ, PDCP drops)
...
LAYER_RANK: Dict[str, int] = {
    ...
    "PDCP":  1,
    "L2":    1,
    "stats": 1,   # stats_detector source maps here
    "RRC":   2,
    "F1AP":  3,
    "E1AP":  4,
    "XnAP":  5,
...
def layer_rank(anomaly: dict) -> int:
    """Return the protocol stack rank of an anomaly (lower = deeper)."""
    layer = anomaly.get("layer") or anomaly.get("source") or "?"
    # stats detector sets source="stats" and no layer field
    if layer == "stats":
        kpi = anomaly.get("kpi", "")
        if any(k in kpi for k in ("bler", "snr", "rsrp", "rsrq", "pusch", "pucch")):
            return LAYER_RANK["PHY"]
        return LAYER_RANK["MAC"]
    return LAYER_RANK.get(layer, 5)


def layer_label(anomaly: dict) -> str:
    layer = anomaly.get("layer") or anomaly.get("source") or "?"
...
class CausalRule:
    name: str
    description: str
    root_layer: str          # layer of the root cause
    effect_layers: List[str] # layers this cause can propagate to
    keywords: List[str]      # root cause anomaly must contain one of these (in type/evidence)
    confidence: float        # base confidence [0, 1]
    spec_ref: str            # 3GPP spec reference
    recommendation: str
...
CAUSAL_RULES: List[CausalRule] = [
    CausalRule(
        ...
        root_layer="PHY",
        effect_layers=["RRC", "NGAP", "NAS", "KPI", "kpi"],
    ...
    CausalRule(
        ...
        root_layer="RRC",
        effect_layers=["NGAP", "NAS", "KPI", "kpi"],
...
def match_rule(anomaly: dict, rule: CausalRule) -> bool:
    """Check if an anomaly matches a causal rule's root cause pattern."""
    text = " ".join([
        anomaly.get("type", ""),
    ...
    layer_matches = (
        ...
        or LAYER_RANK.get(layer, 99) == LAYER_RANK.get(rule.root_layer, -1)
    )
    keyword_matches = any(kw in text for kw in rule.keywords)
    return layer_matches or keyword_matches


def find_rules_for_root(anomaly: dict) -> List[Tuple[CausalRule, float]]:
    """Return (rule, confidence) pairs where this anomaly is a plausible root cause."""
    results = []
    for rule in CAUSAL_RULES:
        if match_rule(anomaly, rule):
            results.append((rule, rule.confidence))
